...
void Individuo :: set_coordinates(vector<int> coordinate){
   for(int i = 0; i < _numero_citta; i++){
      _elemento[i] = coordinate[i];
   }
}
...
void Individuo :: mutation_block_shift(Random& rnd, int block_size, int lunghezza_spostamento){
   int start_index = int(rnd.Rannyu(1, _numero_citta));
   vector<int> appo = _elemento; 
   vector<int> copy(block_size);
   for (int i = 0; i < block_size; i++){copy[i] = _elemento[pbc_index(start_index + i, _numero_citta)];}
   for (int i = 0; i < lunghezza_spostamento; i++){appo[pbc_index(start_index + i, _numero_citta)] = _elemento[pbc_index(start_index + i + block_size, _numero_citta)];}
   for (int i = 0; i < block_size; i++){appo[pbc_index(start_index + i + lunghezza_spostamento, _numero_citta)] = copy[i];}
   _elemento = appo; 
}
...
void Individuo::check_function() {
   ...
   if (errore_trovato) {
   cerr << "======== Integrità dell'Individuo COMPROMESSA ========" << endl << endl;
   }
}
