...
int main (int argc, char *argv[]){
  ...
  cout << " -> Inizio fase di equilibrazione (10000 passi)..." << endl;
  // for(int i=0; i<10000; i++){SYS.step();}       // Fase di equilibrazione
  cout << " -> Equilibrazione completata. Inizio simulazione a blocchi." << endl << endl;
  ...
  // cout << endl << "stampa di DEBUG" << j << endl;
      ...
      if(j%10 == 0){                            // Se j è multiplo di 50 (ogni 50 step) scrivo la configurazione del sistema in formato .xyz
    ...
    cout << "    [PROGRESSO] Blocco " << (i+1) << " di " << SYS.get_nbl() << " completato." << endl;
  }

  cout << endl << " -> Simulazione terminata con successo. Finalizzazione..." << endl;
