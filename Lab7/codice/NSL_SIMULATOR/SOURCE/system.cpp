...
void System :: move(int i){ // Propose a MC move for particle i
  ...
  // Calcoliamo il delta_E associato al cambio di spin
  double delta_E = 2.0 * ( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );
  // Probabilità che lo spin diventi +1
  double p_boltzmann = 1.0 / (1.0 + exp(-_beta * delta_E));
  
  if(_rnd.Rannyu() < p_boltzmann) {
    _particle(i).setspin(1);
  } else {
    _particle(i).setspin(-1);
  }
  _naccepted++; // Nel Gibbs la mossa è sempre "accettata" nel senso che campiona un nuovo stato valido
  } else {           // M(RT)^2
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    // ********* Parte modificata per es. 4.2 da me ********** 

    // Prima opzione da considerare 
    // double vt = sqrt(_temp); // velocità connessa alla radice della temperatura
    // for (int i=0; i<_npart; i++){
    //   int r = int(_rnd.Rannyu(0, 6)); // prendo sei perchè 5.99584 --> 5; 4.2525 --> 4; 3.123423 --> 3 ...

    //   if(r == 0){vx(i) = vt; vy(i) = 0.; vz(i) = 0.;}
    //   else if(r == 1){vx(i) = -vt; vy(i) = 0.; vz(i) = 0.;}
    //   else if(r == 2){vx(i) = 0.; vy(i) = vt; vz(i) = 0.;}
    //   else if(r == 3){vx(i) = 0.; vy(i) = -vt; vz(i) = 0.;}
    //   else if(r == 4){vx(i) = 0.; vy(i) = 0.; vz(i) = vt;}
    //   else if(r == 5){vx(i) = 0.; vy(i) = 0.; vz(i) = -vt;}
    //   sumv(0) += vx(i);
    //   sumv(1) += vy(i);
    //   sumv(2) += vz(i);
    // }

    // Seconda opzione per evitare un drift
    double vt = sqrt(_temp); // velocità connessa alla radice della temperatura
    for (int i = 0; i < _npart; i += 2) {
      int r = int(_rnd.Rannyu(0, 6)); 
      ...
      if(i + 1 < _npart) {vx(i+1) = -vx(i); vy(i+1) = -vy(i); vz(i+1) = -vz(i);}

      // sumv non è più necessario per il controllo del drift perché la somma è zero
    }
    // ****** Fine parte modificata per es. 4.2 da me ******** 


    // ********* Parte modificata per es. 4.3 da me ********** 
    //   ifstream cinf("../INPUT/CONFIG/config.final.xyz"); // O il file dove hai le velocità
    //   string dummy;
    //   double xread, yread, zread, velx, vely, velz;
      
    //   cinf >> _npart >> dummy; // Salta l'header se in formato XYZ

    //   for(int i=0; i<_npart; i++){
    //       // Leggi posizione e velocità finali della simulazione precedente
    //       cinf >> dummy >> xread >> yread >> zread >> velx >> vely >> velz; 

    //       // INVERSIONE: La nuova velocità è quella vecchia col segno MENO
    //       vx(i) = -velx;
    //       vy(i) = -vely;
    //       vz(i) = -velz;
    ...
    //       // Calcola xold per tornare indietro: x(t-dt) = x(t) - (-v)*dt = x(t) + v*dt
    //       // Nota: se inverti v, xold si troverà "nel futuro" rispetto alla direzione originale
    //       _particle(i).setpositold(0, this->pbc( _particle(i).getposition(0,true) + vx(i) * _delta, 0));
    //       _particle(i).setpositold(1, this->pbc( _particle(i).getposition(1,true) + vy(i) * _delta, 1));
    //       _particle(i).setpositold(2, this->pbc( _particle(i).getposition(2,true) + vz(i) * _delta, 2));
    //   }
    //   cinf.close();

    //   return;
    // ****** Fine parte modificata per es. 4.3 da me ******** 
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = ((8. * M_PI * _rho) / (3. * pow(_r_cut, 3))) * ( (1./(3.* pow(_r_cut, 6))) - 1.); // TO BE FIXED IN EXERCISE 7. La epsilon non c'è perchè è in unità di epsilon
        ...
        coutpv << "#  BLOCK      BIN       VELOCITY     AVE_POFV    CUM_POFV      ERROR" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        double v_max = 8.0; // Velocità massima che vogliamo vedere nel grafico
        _bin_size_v = v_max / (double)_n_bins_v;
        _measure_pofv = true;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    cinf >> ncoord;
    if (ncoord != _npart){
      cerr << "PROBLEM: conflicting number of coordinates in input.dat & config.xyz not match!" << endl;  // uguale
      exit(EXIT_FAILURE);
    }
    cinf >> comment;
    for(int i=0; i<_npart; i++){
      ...
      // Se riparti prendi in ingresso la configurazione così com'è
      if(_restart){
      ...
      // QUI è DIVERSO RISPETTO AL LAB. 4
    // altrimenti dividi per 2 (come richiesto nell'es 4.2 e 4.3)
    else{
      _particle(i).setposition(0, this->pbc(_side(0)*x, 0));
      _particle(i).setposition(1, this->pbc(_side(1)*y, 1));
      _particle(i).setposition(2, this->pbc(_side(2)*z, 2));
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if (_measure_gofr){
          int bin_gofr = int(dr / _bin_size); // stessa identica logia della pofv
          if (bin_gofr < _n_bins){_measurement(_index_gofr + bin_gofr) += 2.0;} // Incremento di 2
        }
  ...
  // Calcolo della magnetizzazione totale del sistema
  ...
  if (_measure_pofv) {                                                                   // Se ho il flag pofv
    double v;                                                                            // contenitore per le velocità
    for (int i=0; i<_npart; i++){                                                        // ciclo su tutte le particelle
      v = sqrt( dot(_particle(i).getvelocity(), _particle(i).getvelocity()) );           // calcolo la velocità di ogni particella
      bin = int(v/_bin_size_v);                                                          // questo è il bin in cui va a finire la velocità v che ho calcolato
      if (bin < _n_bins_v) _measurement(_index_pofv + bin) += 1.0;                       // se il bin è minore del numero di bin, incremento il contatore del bin corrispondente. _measurement è un vettore che poi sotto le opportune normalizzazioni andrà a comporre il mio istogramma
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
...
void System :: averages(int blk){
  ...
  // =========================================================================
  // METTI IL NUOVO BLOCCO ESATTAMENTE QUI:
  // =========================================================================
  if (_measure_cv){
    double h2_media = _average(_index_cv);          // <H^2> del blocco attuale
    double h_media  = _average(_index_tenergy);     // <H> del blocco attuale
    
    // Trasformiamo la misura grezza nel VERO Calore Specifico del blocco
    _average(_index_cv) = _beta * _beta * double(_npart) * (h2_media - h_media * h_media);
  }
  // =========================================================================
  ...
  if (_measure_gofr){
    ...
    for (int i = 0; i < _n_bins; i++) {
      ...
      double r_plus_dr = r + _bin_size;    
      double delta_V_gofr = (4./3.)*M_PI*(pow(r_plus_dr, 3) - pow(r, 3));
      double norm_gofr = 1./(_rho * double(_npart) * delta_V_gofr);
      average  = _average(_index_gofr + i);
      sum_average = _global_av(_index_gofr + i);
      sum_ave2 = _global_av2(_index_gofr + i);
      // Distanza da stampare nel file (centro del bin)
      double r_center = r + (_bin_size / 2.0);
      
      coutf << setw(10) << blk                                                  // Blocco corrente
            << setw(15) << i                                                    // Indice bin
            << setw(15) << r_center                                             // Distanza (r)
            << setw(20) << average * norm_gofr                                  // g(r) del blocco attuale
            << setw(20) << (sum_average / double(blk)) * norm_gofr              // g(r) media cumulativa
            << setw(20) << this->error(sum_average, sum_ave2, blk) * norm_gofr  // Errore statistico
            << endl;
    }
    coutf.close();
  }
  ...
  if (_measure_pofv){
    ...
    // devo calcolare anche la normalizzazione dell'istogramma
    double norm = 1.0 / (double(_npart) * _bin_size_v);
    for (int i=0; i<_n_bins_v; i++){
      average  = _average(_index_pofv + i);                                 // average è un vettore che contiene tutte le medie dei blocchi per tutte le proprietà. _index_pofv è l'indice a cui inizia la proprietà pofv, quindi _average(_index_pofv + i) è la media del blocco per il bin i-esimo della pofv
      sum_average = _global_av(_index_pofv + i);                            // global_av è un vettore che contiene la somma cumulativa delle medie dei blocchi per tutte le proprietà. _global_av(_index_pofv + i) è quindi la somma cumulativa delle medie dei blocchi per il bin i-esimo della pofv
      sum_ave2 = _global_av2(_index_pofv + i);                              // global_av2 è un vettore che contiene la somma cumulativa delle medie dei blocchi al quadrato per tutte le proprietà. _global_av2(_index_pofv + i) è quindi la somma cumulativa delle medie dei blocchi al quadrato per il bin i-esimo della pofv
      coutf << setw(8)  << blk                                              // 1. Numero del blocco corrente
            << setw(8)  << i                                                // 2. Indice del bin (0, 1, 2...)
            << setw(15) << (i + 0.5) * _bin_size_v                          // 3. Velocità (centro bin)
            << setw(15) << average * norm                                   // 4. P(v) del blocco attuale
            << setw(15) << (sum_average / double(blk)) * norm               // 5. P(v) media cumulativa
            << setw(15) << this->error(sum_average, sum_ave2, blk) * norm   // 6. Errore statistico
            << endl;
    }
    coutf.close();
  }
  ...
  if (_measure_magnet){
    ...
    coutf << setw(10) << blk
          ...
          << setw(20) << this->error(sum_average, sum_ave2, blk) << '\n';
    coutf.close();
  }
  // SPECIFIC HEAT ///////////////////////////////////////////////////////////// // TO BE FIXED IN EXERCISE 6
